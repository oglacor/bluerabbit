<div class="layer base relative lore-item">
	...
	<div class="icon-group layer base w-full realtive padding-0">
		<?php if($b->quest_style =='resource'){ ?>
			...
			<div class="icon-content white-color br-text-24 w700">
				<div class="icon-content white-color">
					<div class="line br-text-24 w700">
						<a href="<?= $b->quest_secondary_headline;?>" target="_blank" title="<?= __("Download","bluerabbit"); ?>">
							<?=$b->quest_title;?>
						</a>
					</div>
					<div class="line br-text-16 w300 max-h-32 overflow-hidden"><?= wp_trim_words($b->quest_content, 20);?></div>
				</div>
			</div>
			<div class="icon-content white-color br-text-16 w700">
		...
		<?php }elseif($b->quest_style =='article'){  ?>
			...
			<div class="icon-content white-color">
				<div class="line br-text-24 w700"><?=$b->quest_title;?></div>
				<div class="line br-text-16 w300 max-h-32 overflow-hidden"><?= wp_trim_words($b->quest_content, 20);?></div>
			</div>
			<div class="icon-content white-color br-text-16 w700">
		...
		<?php if($isGM || $isAdmin){ ?>
			<div class="icon-content">
				<a class="form-ui br-text-16 green-bg-200 w-100 white-color text-center" href="<?= get_bloginfo('url')."/new-lore/?adventure_id=$adventure->adventure_id&questID=$b->quest_id"; ?>">
					<span class="icon icon-edit"></span><?= __("Edit","bluerabbit"); ?>
				</a>
				<br>
				<button class="form-ui br-text-16 red-bg-200 w-100 white-color text-center" onClick="showOverlay('#confirm-trash-<?= $b->quest_id; ?>');">
