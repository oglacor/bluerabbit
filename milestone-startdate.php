	<div class="milestone <?php echo "$mi->quest_type $hideByDay $left_side level{$mi->mech_level}";  ?>"  id="milestone-<?= $elementID; ?>" style="<?= $scale; ?>">
		...
		<input type="hidden" class="milestone-data-color" value="cyan">
		<input type="hidden" class="milestone-data-type" value="startdate">
        <div class="milestone-modal-content">
            ...
            <div class="milestone-action">
                <button class="action-button startdate">
                    <?= __("Available on: ","bluerabbit")." ".date('D, M jS, Y | H:i',strtotime($mi->mech_start_date)); ?>
                </button>
            </div>
		...
		<div class="milestone-cta">
			...
			<div class="milestone-preview-title">
				...
				<h2 class="cyan-bg-A400 blue-grey-900 padding-5">
					<span class="icon icon-lock inline-block"></span> 
					<?= __("Available on: ","bluerabbit")." ".date('D, M jS, Y | H:i',strtotime($mi->mech_start_date)); ?>
				</h2>
			</div>
		</div>
		<div class="milestone-bg-color cyan-bg-A400"></div>
		<div class="milestone-bg-badge cyan-bg-900 blend-luminosity" style="background-image: url(<?= $mi->mech_badge; ?>);">
			<span class="icon icon-lock cyan-A400 font _30 inline-block absolute perfect-center block"></span>
			<?php if($isAdmin || $isGM){ ?>
