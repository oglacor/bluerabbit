<div class="w-full padding-10">
	<?php if($q['survey_question_display'] =='dropdown'){ ?>
		<div class="input-group font _24 w900 text-center w-200 boxed">
			...
			<select class="form-ui range-value font _24 w900 white-bg" id="question-answer-value-<?php echo $i; ?>" onChange="submitSurveyAnswer(<?php echo $i; ?>);">
				<?php for($j=0; $j<=$q['range']; $j++){ ?>
					<option value="<?= $j; ?>" <?php echo ($j==$q['survey_answer_value']) ? 'selected' : ''; ?>><?= $j; ?></option>
				<?php } ?>
			</select>
		</div>
	...
	<?php }else{ ?>
		<div class="input-group font _24 w900 text-center w-200 boxed">
			<label class="deep-purple-bg-400"><?php _e("Value","bluearbbit"); ?>: </label>
			<input type="number" id="question-answer-value-<?php echo $i; ?>" value="<?= $q['survey_answer_value']; ?>" class=" text-center form-ui range-value white-bg" onChange="submitSurveyAnswer(<?php echo $i; ?>);">
		</div>
	<?php } ?>
</div>
