...
$reward_item_id = $step->step_item_reward ?: ($settings['item_id'] ?? $step->step_item);
$item = $reward_item_id ? $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}br_items WHERE item_id = %d", $reward_item_id)) : null;
$pickupNonce = wp_create_nonce('pickup_item'.$current_user->ID.date('Ymd'));
?>
<div class="step <?= $i==0 ? 'active' : ''; ?>" id="step-<?= $step->step_order; ?>">
	...
	<div class="step-content-container grab-item">
		...
		<?= BR_Utils::instance()->stepTag(__("You found an item on your path!","bluerabbit")); ?>
		<?php if ($item) { ?>
				...
				<div class="item-content-container">
					...
					<div class="item-content">
						<div class="item-image-container">
							<img class="item-image" src="<?= $item->item_badge; ?>" alt="<?= esc_attr($item->item_name); ?>">
						</div>
						...
						<h2 class="item-name"><?= esc_html($item->item_name); ?></h2>
					</div>
				</div>
			</div>
		</div>
		<div class="action-buttons">
			...
			<?php } else { ?>
				<a class="action-button warning" href="#step-<?= $steps[($i+1)]->step_order; ?>" onClick="pickupItem(<?=$item->item_id;?>,'<?= $pickupNonce; ?>');">
