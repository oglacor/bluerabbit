...
<?php 
	...
	WHERE quests.quest_type='survey' AND quests.adventure_id=$adventure->adventure_id AND quests.quest_status='publish'
		...
		<div class="highlight teal-bg-800 padding-10 page-break text-center">
			<div class="icon-group">
				...
				<span class="icon-content white-color">
					<span class="line font _36 w300"> <?= __("Surveys","bluerabbit"); ?> </span>
					<span class="line font _16 w300"> <?= __("A list to all survey results","bluerabbit"); ?> </span>
				</span>
			</div>
		</div>
		<?php if($quests){ ?>
			<table class="table">
				...
				<tbody>
					...
					<tr>
						<td><?= $q->quest_id; ?></td>
						<td><a target="_blank" href="<?= get_bloginfo('url')."/survey-results/?questID={$q->quest_id}&adventure_id={$adventure->adventure_id}"; ?>"><?= $q->quest_title; ?></a></td>
