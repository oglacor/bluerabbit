...
class BR_Progression {
    ...
    public function getPlayerProgress($adventure_id, $uID){
        global $wpdb;
        $user=BR_Player::instance()->getPlayerData($uID);
        $data = array();
        $data['success']=false;
     ...
     */
    public function getNextAvailableMilestone($adv_parent_id, $adv_child_id, $current_quest_id, $adventure, $playerState) {
        global $wpdb;
    ...
    public function getRequirements($quest_id, $adv_id){
        ...
        if($quest){
            ...
            $p_posts = BR_Quest::instance()->getMyQuests($adv_child_id); // returns as array with IDs
            $p_achievements = BR_Achievement::instance()->getMyAchievements($adv_child_id); // returns IDs in one Array
            $p_items = BR_Item::instance()->getMyItems($adv_child_id); // returns as array with types [key,consumable,reward]
