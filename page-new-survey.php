...
if(isset($quest)){
	$all_qs = $wpdb->get_results("SELECT a.*, b.survey_option_id, b.survey_option_text, b.survey_option_image FROM {$wpdb->prefix}br_survey_questions a LEFT JOIN {$wpdb->prefix}br_survey_options b ON a.survey_question_id = b.survey_question_id AND b.survey_option_status='publish' WHERE a.survey_id=$quest->quest_id AND a.survey_question_status='publish' ORDER BY a.survey_question_order, a.survey_question_id");
	$questions = array();
	...
	foreach($requirements as $r){
		...
		else if ($r->req_type=='achievement') $reqs['achievements'][]=$r->req_object_id;
	}
}
...
<div class="br-page br-has-bottom-bar">
	...
	<div class="br-panel br-page-header">
		...
		<input type="hidden" id="the_survey_id" value="<?= $is_edit ? $quest->quest_id : ''; ?>">
	...
	<div class="br-tabs br-tabs-sticky" id="tab-group-buttons">
		...
		<button class="br-tab-btn" onClick="brScrollTo('survey-questions', this);">
	...
	<!-- Tab Content -->
		...
		<!-- ═══ MECHANICS ═══ -->
		<div class="br-scroll-section" id="mechanics"><div class="br-panel">
		...
		<div class="br-scroll-section" id="survey-questions"><div class="br-panel">
			<div class="padding-10 grey-bg-200">
				<span class="icon-group">
					...
					<span class="icon-content pull-right">
						...
						<button class="form-ui red-bg-400 font condensed _18 w300 reorder-actions hidden" onClick="deactivateReorder('#questions');"><span class="icon icon-cancel"></span> <?php _e("Cancel Reorder","bluerabbit"); ?></button>
						<button class="form-ui amber-bg-400 font condensed _18 w300 reorder-actions hidden pull-right" onClick="reorderQuestions('#questions');"><span class="icon icon-list"></span> <?php _e("Save order","bluerabbit"); ?></button>
					</span>
				</span>
			</div>
			<?php if(isset($adventure) && isset($quest)){ ?>
				<div class="add-new-question-header">
					...
					<button class="form-ui teal-bg-400" onClick="addQuestion('survey');"><?php _e("Closed","bluerabbit"); ?></button>
					<button class="form-ui red-bg-400" onClick="addQuestion('survey','multi-choice');"><?php _e("Multi Choice","bluerabbit"); ?></button>
					<button class="form-ui blue-bg-400" onClick="addQuestion('survey','open');"><?php _e("Open","bluerabbit"); ?></button>
					<button class="form-ui amber-bg-400" onClick="addQuestion('survey','rating');"><?php _e("Rating","bluerabbit"); ?></button>
					<button class="form-ui purple-bg-400" onClick="addQuestion('survey','number');"><?php _e("Value","bluerabbit"); ?></button>
					<?php if($use_guilds){ ?>
						<button class="form-ui light-green-bg-400" onClick="addQuestion('survey','guild-vote');"><?php _e("Guild Vote","bluerabbit"); ?></button>
					<?php } ?>
				</div>
				<div class="w-full">
					<ul class="questions accordion" id="questions">
						<?php if($questions && $quest->quest_id){ foreach($questions as $qKey=>$question){ include (TEMPLATEPATH . '/survey-question-form.php'); } } ?>
					</ul>
				</div>
